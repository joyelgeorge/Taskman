...
test('high-priority revenue issues outrank ordinary and non-revenue work', () => {
  const issues = [
    { number: 39, title: 'Observability: trace pipeline progress', labels: [] },
    { number: 43, title: 'Revenue Rail: DeskCrew x402 support-bounty rail', labels: [{ name: 'high-priority' }] },
    { number: 45, title: 'Fix poller prioritization', labels: [{ name: 'high-priority' }] }
  ];

  const picked = selectActionableIssues(issues, [], 3);
  assert.equal(picked[0].number, 43, 'DeskCrew revenue rail must be ranked #1');
});
