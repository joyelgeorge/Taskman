...
test('every table the code writes to actually exists', dbOnly, async () => {
  ...
  const required = [
    ...
    'bounty_candidates', 'bounty_triage_records', 'scanned_repos', 'outreach_attempts'
  ];
  const { rows } = await query(
...
test('outreach_attempts.outcome accepts every outcome the log can record', dbOnly, async () => {
  const allowed = await allowedValues('outreach_attempts', 'outcome');
  const rejected = Object.values(OUTREACH_OUTCOME).filter(v => !allowed.has(v));
  assert.deepEqual(rejected, [],
    `outreach_attempts.outcome would reject ${JSON.stringify(rejected)} — the log produces these, `
    + `the schema permits only ${JSON.stringify([...allowed])}.`);
});
