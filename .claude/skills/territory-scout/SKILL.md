...
2. **Widen deliberately.** Draw candidates from directions the fleet does not
   already cover. Examples of *directions*, not answers: data/signal products
...
3. **Score each** with `packages/core/territory/scoring.js` by choosing the
   ...
   - `railFitIndia`: paypal_or_bank | crypto | stripe_express | closed  (closed is fatal)
