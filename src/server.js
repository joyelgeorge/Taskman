...
import { executeWithConcurrencyPolicy, executeBrainWithConcurrencyPolicy, CONCURRENCY_POLICY } from './scheduler-concurrency.js';
...
function schedule(task) {
  ...
  const timer = setInterval(() => {
    executeWithConcurrencyPolicy(task, t => executeTask(t, 'schedule'), { policy: task.concurrencyPolicy || CONCURRENCY_POLICY.FORBID }).catch(console.error);
  }, check.value * 60_000);
  timer.unref();
...
function startBrainScheduler() {
  ...
  brainTimer = setInterval(() => {
    executeBrainWithConcurrencyPolicy(() => executeBrainCycle('schedule'), { policy: CONCURRENCY_POLICY.FORBID }).catch(console.error);
  }, minutes * 60_000);
  brainTimer.unref();
  console.log(`Taskman brain scheduler enabled: every ${minutes} minute(s) (concurrency: FORBID)`);
}
...
import { getPipelineObservabilitySummary } from './pipeline-observability.js';
