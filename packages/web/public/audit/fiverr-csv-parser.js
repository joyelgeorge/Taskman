/**
 * Resolved at load rather than imported statically, so this module also loads in
 * a browser. The audit runs client-side on the public page — the files never
 * leave the machine — and that is only possible if the parser has no hard Node
 * dependency. The hash is provenance for a server-side run; where it is
 * unavailable the field is null and nothing else changes.
 */
let createHash = null;
try {
  ({ createHash } = await import('node:crypto'));
} catch {
  // Browser: no node:crypto. Parsing does not depend on it.
}
const sha256 = text => (createHash ? createHash('sha256').update(text).digest('hex') : null);
