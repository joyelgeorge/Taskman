...
function isRevenueIssue(issue) {
  const text = `${issue.title || ''}\n${issue.body || ''}`.toLowerCase();
  const labels = new Set((issue.labels || []).map(normalizeLabel));
  return (
    labels.has('revenue') ||
    labels.has('revenue-rail') ||
    labels.has('monetization') ||
    labels.has('bounty') ||
    text.includes('revenue rail') ||
    text.includes('deskcrew') ||
...
function priorityRank(issue) {
  const labels = new Set((issue.labels || []).map(normalizeLabel));
  const isHighPriority = labels.has('high-priority') || labels.has('high priority') || labels.has('priority: high') || labels.has('p0') || labels.has('priority: critical') || labels.has('critical');
  const isRev = isRevenueIssue(issue);

  // Precedence:
  // 1. High-priority revenue issue (Rank 10)
  // 2. High-priority blocker / reliability issue (Rank 8)
  // 3. Ordinary revenue issue (Rank 6)
  // 4. P1 / High priority ordinary (Rank 4)
  // 5. P2 / Medium (Rank 2)
  // 6. Ordinary (Rank 0)
  if (isHighPriority && isRev) return 10;
  if (isHighPriority) return 8;
  if (isRev) return 6;
  if (labels.has('p1')) return 4;
  if (labels.has('p2') || labels.has('priority: medium')) return 2;
  return 0;
}
